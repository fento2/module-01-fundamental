...

{
    ...
    const input : number = 15;                          //👈input user
    let print : string = "";                            //sama kayak di atas
    let fn : number = 0;                                //variabel fn buat nyimpan hasil ke-nnya
    let f1 : number = 0;                                //variabel f1 menjadi awalan
    let f2 : number = 1;                                //sama kayak variabel f1
    let temp :number = 0;                               //variabel sementara buat simpan f1

    for (let i : number = 1; i<=input; i++){            //for loop buat ngitung deret fibonaci ke-nnya dari input user
        fn = f1 + f2;                                   //yah gini lah kira kira penjumlahanya
        temp = f1;                                      //karna harus ditambah 2 angka sebelumnya jadi kita simpan nilai f1 ke temp
        f1 = fn;                                        //nah f1 kita kasi nilai fn hasil f1+f2
        f2 = temp;                                      //f2-nnya kita isi sama  nilai f1 yang disimpan di temp tadi                                      
    }                                                   //dah gitu terus sampe kondisinya terpenuhi
    print += `${input} → ${fn}`                         //variabel printnya diisi input sama hasilnya
    console.log(print);
}           //nih hasilnya 👆
